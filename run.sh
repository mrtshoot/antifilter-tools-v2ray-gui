...
func_exec_script() {
    ansible-playbook -i inventory playbook.yaml --extra-vars="api_token='${api_token}' domain='${domain}' subdomain='${subdomain}' ip_address='${SERVER_IP}' proxied='${proxied}' config_account='${config_account}' config_password='${config_password}' config_port='${config_port}' email='${email}'"
}

func_exec_script "$api_token $domain $subdomain $SERVER_IP $proxied $config_account $config_password $config_port $email"